...
# case 4: 
# action: do full backup -> update tuple -> do block backup -> recovery through full backup and block backup
...
rm -rf $PGDATA/test.sql
touch $PGDATA/test.sql
echo "select relid from pg_stat_all_tables where relname='employees';" >> $PGDATA/test.sql
echo 'update employees set salary = 9999 where id = 1;' >> $PGDATA/test.sql
echo 'update employees set salary = 9999 where id = 500;' >> $PGDATA/test.sql
echo 'update employees set salary = 9999 where id = 1000;' >> $PGDATA/test.sql
cat $PGDATA/test.sql
psql -d polardb -U polardb -f $PGDATA/test.sql > $PGDATA/genlogfile
