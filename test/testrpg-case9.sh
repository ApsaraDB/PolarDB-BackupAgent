...
# case 9: 
# action: do full backup -> insert tuples -> do block backup -> insert tuples -> do block backup -> insert tuples -> wal backup -> recovery through full backup and block backup and wals by fs
...
rm -rf $PGDATA/test.sql
touch $PGDATA/test.sql
echo 'insert into employees SELECT generate_series(1001,2000) as key,repeat( chr(int4(random()*26)+65),4), (random()*(6^2))::integer,null,(random()*(10^4))::integer;' >> $PGDATA/test.sql
cat $PGDATA/test.sql
psql -d polardb -U polardb -f $PGDATA/test.sql > $PGDATA/genlogfile
cat $PGDATA/genlogfile
sleep 1
...
echo ""
echo "BlockBackup-'$date3'"
echo ""
read -p "enter to continue" 

echo '########## make another change again ##########'
rm -rf $PGDATA/test.sql
touch $PGDATA/test.sql
echo 'insert into employees SELECT generate_series(2001,3000) as key,repeat( chr(int4(random()*26)+65),4), (random()*(6^2))::integer,null,(random()*(10^4))::integer;' >> $PGDATA/test.sql
...
curl "http://127.0.0.1:1888/manager/Recovery" -d '{
  ...
  "Incremental": {"InstanceID": "'$Instance'", "BackupID": "IncreBackup"},
...
sleep 1
rm -rf ~/test.sql
echo 'excute sql done, output should be 3000!'
cat ~/genlogfile
rm ~/genlogfile
echo ""
read -p "enter to continue" 