...
# action: do full backup -> insert some data -> wait wals to be backuped -> recovery specify full backup
# change: add namespace for dbs
...
curl "http://127.0.0.1:1888/manager/StartBackup" -d '{
  ...
  "Filesystem":"pfs.file",
  "BackupStorageSpace": {"StorageType":"dbs", "Locations":{"dbs": {"Endpoint": "'$backupgateway'","Namespace":"backuptest"}}}
}'
sleep 2
echo ""
read -p "enter to continue" 
...
curl "http://127.0.0.1:1888/manager/Recovery" -d '{
  ...
  "BackupPBD": "'$dstdev'",
  "BackupMetaSource": "db",
  "Full": {"InstanceID": "'$Instance'", "BackupID": "FullBackup-'$date1'"},
  "RecoveryTime": 0,
  "BackupAccount": {"User": "polardb","Password": "cG9sYXJkYg==","Endpoint": "127.0.0.1,198.19.64.3,198.19.64.2","Port": "5432"},
  "Filesystem":"pfs.file",
  "DBClusterMetaDir": "",
