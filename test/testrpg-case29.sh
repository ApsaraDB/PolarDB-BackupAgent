...
# case 29: 
# action: do full backup -> insert some data -> do block backup and abort just before resetblocksync function -> do block backup again -> recovery through full backup and block backup
...
echo '########## config backup ##########'
echo polardb1234 | sudo -S sed -i "s/\"EnableBlockBackup\": false/\"EnableBlockBackup\": true/g" /usr/local/polardb_o_backup_tool_current/bin/pgpipeline.conf
echo polardb1234 | sudo -S sed -i "s/\"EnableBlockBackup\": false/\"EnableBlockBackup\": true/g" /usr/local/polardb_o_backup_tool_current/bin/increpipeline.conf

echo '########## restart backup agent ##########'
echo polardb1234 | sudo -S systemctl restart backupctl.service
sleep 1
...
echo 'insert into employees SELECT generate_series(0,110) as key,repeat( chr(int4(random()*26)+65),4), (random()*(6^2))::integer,null,(random()*(10^4))::integer;' >> $PGDATA/test.sql
cat $PGDATA/test.sql
...
echo '########## wait to restart agent ##########'
echo ""
read -p "enter to restart agent" 
echo polardb1234 | sudo -S systemctl restart backupctl.service
sleep 1

echo '########## start wal backup again ##########'
curl "http://127.0.0.1:1888/manager/StartBackup" -d '{
  "GatewayAddressList": [""],
  "BackupMachineList": ["http://127.0.0.1:1888"],
  "InstanceID": "'$Instance'",
...
sleep 1
rm -rf ~/test.sql
echo 'excute sql done, output should be 111!'
cat ~/genlogfile
rm ~/genlogfile
echo ""
read -p "enter to continue" 