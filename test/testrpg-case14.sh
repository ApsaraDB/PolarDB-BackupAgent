...
# case 14: 
# action: do full backup in cmd mode with selfdefine cryption -> insert tuples -> wal backup in cmd mode with selfdefine cryption 
# -> recovery through full backup and wals by fs with wrong encrytion password
# expect: restore will be failed since encryption password is wrong
...
./backup_ctl -config $workdir -instance "$Instance-full" -plugin pgpipeline -fs '{
    ...
    "EncryptionPassword": "testpass1234",
    "Compress": true,
    "WorkerCount": 16,
    "Action": "backup",
    "HostIP": "127.0.0.1",
...
curl "http://127.0.0.1:1888/manager/Recovery" -d '{
  "UseBlock": true,
  "EnableEncryption": true,
  "EncryptionPassword": "testwrongpass",
