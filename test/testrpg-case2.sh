...
# case 2: 
# action: do full backup -> delete some data to trigger file truncate -> do block backup -> recovery through full backup and block backup
...
rm -rf $PGDATA/test.sql
touch $PGDATA/test.sql
echo 'delete from employees where id >= 2 and id <= 1000;' >> $PGDATA/test.sql
echo 'vacuum;' >> $PGDATA/test.sql
cat $PGDATA/test.sql
psql -d polardb -U polardb -f $PGDATA/test.sql > $PGDATA/genlogfile
sleep 1
rm -rf $PGDATA/test.sql
