...
# case 5: 
# action: do full backup -> drop database -> do block backup -> recovery through full backup and block backup
...
echo 'create database testdb;' >> $PGDATA/test.sql
cat $PGDATA/test.sql
...
rm -rf $PGDATA/test.sql
touch $PGDATA/test.sql
echo "drop database testdb;" >> $PGDATA/test.sql
cat $PGDATA/test.sql
psql -d polardb -U polardb -f $PGDATA/test.sql > $PGDATA/genlogfile
