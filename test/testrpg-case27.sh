...
# case 27: 
# action: do full backup -> insert some data -> do wal backup -> recovery through full backup and wals
# enviroment: test rw node for flex
# prepare: http_file_server should be running
# interface: backup from cm and recovery from agent
...
' >> ~/test.sql
echo 'select pg_switch_wal();' >> ~/test.sql
cat ~/test.sql
psql -h 127.0.0.1 -p 1521 -d polardb -U polardb -f ~/test.sql > ~/genlogfile
sleep 1
rm -rf ~/test.sql
...
echo '########## change backup conf ##########'
echo Ali165243 | sudo -S sed -i "s/172.16.241.184:5500/172.16.241.184:1888/g" /usr/local/polardb_o_backup_tool_current/bin/pgpipeline.conf
echo Ali165243 | sudo -S sed -i "s/172.16.241.184:5500/172.16.241.184:1888/g" /usr/local/polardb_o_backup_tool_current/bin/increpipeline.conf
echo Ali165243 | sudo -S systemctl restart backupctl.service
sleep 2

echo '########## recovery full and wals ##########'
curl "http://$testnode:1888/manager/Recovery" -d '{
...
echo '########## reset backup conf ##########'
echo Ali165243 | sudo -S sed -i "s/172.16.241.184:1888/172.16.241.184:5500/g" /usr/local/polardb_o_backup_tool_current/bin/pgpipeline.conf
echo Ali165243 | sudo -S sed -i "s/172.16.241.184:1888/172.16.241.184:5500/g" /usr/local/polardb_o_backup_tool_current/bin/increpipeline.conf
echo Ali165243 | sudo -S systemctl restart backupctl.service
...
sleep 1
rm -rf ~/test.sql
echo 'excute sql done, output should be 1001!'
cat ~/genlogfile
rm ~/genlogfile
echo ""
read -p "enter to continue" 
