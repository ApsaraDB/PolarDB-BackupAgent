...
# case 15: 
# action: do full backup -> pgbench for 100 wals -> delete some data -> do block backup
# -> compare time between recovery through full backup and block backup/ wals
...
echo '########## run pgbench ##########'
echo 'wait pgbench to produce 100w wals'
echo ""
read -p "enter to continue" 
...
echo "recovery_target_time = '2022-09-16 16:01:40'" >> $PGDATA/recovery.conf
echo "restore_command = 'echo not copy'" >> $PGDATA/recovery.conf
cat $PGDATA/recovery.conf

echo '########## start pg ##########'
pg_ctl start -D $PGDATA
echo "calculate time of wal recovery"
echo ""
read -p "enter to continue" 
