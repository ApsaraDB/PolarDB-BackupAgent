...
# case 26: 
# action: do full backup and abort after agent do pgstartbackup -> do full backup again -> insert some data -> backup wals 
# -> recovery through full backup and wals
...
echo 'insert into employees SELECT generate_series(0,1000) as key,repeat( chr(int4(random()*26)+65),4), (random()*(6^2))::integer,null,(random()*(10^4))::integer;' >> $PGDATA/test.sql
