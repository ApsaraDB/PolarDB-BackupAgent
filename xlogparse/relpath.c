...
/* POLAR */
bool		polar_enable_shared_storage_mode = true;
char		*polar_datadir = "polar_datadir";
				...
				int backendId, ForkNumber forkNumber)
{
	...
	if (spcNode == GLOBALTABLESPACE_OID)
	{
		...
		if (forkNumber != MAIN_FORKNUM)
			sprintf(path, "global/%u_%s",
							relNode, forkNames[forkNumber]);
		else
			sprintf(path, "global/%u", relNode);
	}
	else if (spcNode == DEFAULTTABLESPACE_OID)
	{
		/* The default tablespace is {datadir}/base */
		...
		{
			if (forkNumber != MAIN_FORKNUM)
				sprintf(path, "base/%u/t%d_%u_%s",
								dbNode, backendId, relNode,
								forkNames[forkNumber]);
			else
				sprintf(path, "base/%u/t%d_%u",
								dbNode, backendId, relNode);
		}
	}
	else
	{
		...
		{
			if (forkNumber != MAIN_FORKNUM)
				sprintf(path, "pg_tblspc/%u/%s/%u/%u_%s",
								spcNode, TABLESPACE_VERSION_DIRECTORY,
								dbNode, relNode,
								forkNames[forkNumber]);
			else
				sprintf(path, "pg_tblspc/%u/%s/%u/%u",
								spcNode, TABLESPACE_VERSION_DIRECTORY,
								dbNode, relNode);
		}
		else
		{
			if (forkNumber != MAIN_FORKNUM)
				sprintf(path, "pg_tblspc/%u/%s/%u/t%d_%u_%s",
								spcNode, TABLESPACE_VERSION_DIRECTORY,
								dbNode, backendId, relNode,
								forkNames[forkNumber]);
			else
				sprintf(path, "pg_tblspc/%u/%s/%u/t%d_%u",
								spcNode, TABLESPACE_VERSION_DIRECTORY,
								dbNode, backendId, relNode);
		}
	}
		...
		POLAR_NORMAL_TABLE_FILE_IN_SHARED_STORAGE(backendId))
	{
		...
		sprintf(newpath, "%s/%s", polar_datadir, path);
		// pfree(path);
		free(path);
		return newpath;
	}
