...
#define palloc_extended(x, y) malloc(x)
...
polar_xlog_read_record(XLogReaderState *state, XLogRecPtr RecPtr, char **errormsg)
{
	...
	bool		gotheader;
	int			readOff;
	char			*xlog_block_start_buffer = NULL, *xlog_record_buffer = NULL;
	uint32		gotlen = 0;
	...
	*errormsg = NULL;
	...
	if (polar_enable_redo_hashtable)
	{
		/*
		 * Wait util enough xlog record buffer coming as needed.
		 */
		do
		{
			xlog_block_start_buffer = polar_get_free_xlog_block_buffer(total_len);
			if (xlog_block_start_buffer == NULL)
			{
				pg_usleep(1000 * 1000L);
				CHECK_FOR_INTERRUPTS();
				/* Handle interrupt signals of startup process */
				HandleStartupProcInterrupts();
				polar_handle_walreceiver_die(targetPagePtr, RecPtr);
			}
			/* We try again after release xlog buffer if buffer is null */
		} while(xlog_block_start_buffer == NULL);

		xlog_record_buffer = xlog_block_start_buffer;

		/* skip shared mem xlog record block head */
		xlog_record_buffer += sizeof(XLogBlock);
	}
	else
	{
		/*
	...
	/* set XLogBlock header metainfo */
	if (polar_enable_redo_hashtable)
		polar_init_xlog_block_data(xlog_block_start_buffer, state->ReadRecPtr, state->EndRecPtr);
